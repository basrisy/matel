        ...
        <table id="tabledata" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php if ($masaberlaku/(24*60*60) < 0) { echo "0 Hari"; } else { echo number_format($sisahari)." Hari"; }?></td>     
                    <td><?php if($key->status_aktif == 0) { echo "Aktif";} else if($sisahari > 1){ echo "User Blokir";} else { echo "Tidak Aktif";} ?></td>       
                    <td style="vertical-align:middle;text-align:right">
                    <?php if($key->status_aktif == 1 && $sisahari < 1 ){ ?>
                        <a data-toggle="modal" data-target="#terima<?=$key->id;?>" class="btn btn-xs btn-success" title="Terima"><i class="fa fa-check-square-o"></i> Terima</a>
                        <a href="<?=base_url();?>user/hapus_user/<?=$key->id;?>" class="btn btn-xs btn-warning" title="Tolak"><i class="fa fa-times"></i> Tolak</a>
                    <?php } ?>
                    <?php if($key->status_aktif == 0 ){ ?>
                        <a href="<?=base_url();?>user/status/<?=$key->id;?>" class="btn btn-xs btn-dark" title="Blokir User"><i class="fa fa-lock"></i> Blokir User</a>
                    <?php } else if($key->status_aktif == 1 && $sisahari > 1){ ?>
                        <a data-toggle="modal" data-target="#terima<?=$key->id;?>" class="btn btn-xs btn-primary" title="Aktifkan"><i class="fa fa-unlock"></i> Aktifkan</a>
                        <a href="<?=base_url();?>user/hapus_user/<?=$key->id;?>" class="btn btn-xs btn-danger" title="Hapus" onclick="return confirm('Yakin Ingin Menghapus Data ini ?')"><i class="fa fa-trash"></i> Hapus</a>
                    <?php } ?>
                    </td>
